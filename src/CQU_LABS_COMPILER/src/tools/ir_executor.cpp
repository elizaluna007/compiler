    ...
    for(const auto& gte: program->globalVal) {
        ...
        if (gte.maxlen) {
            if (gte.val.type == Type::IntPtr) {
                ...
                if(gte.val.name == "array:global") printf("address: %p\n",entry.second._val.iptr);
                for(size_t i = 0; i < gte.maxlen; i++) entry.second._val.iptr[i] = 0;
            }
            else if (gte.val.type == Type::FloatPtr) {
                entry.second._val.fptr = new float[gte.maxlen];    
                for(size_t i = 0; i < gte.maxlen; i++) entry.second._val.fptr[i] = 0;
            }
            else {
...
bool ir::Executor::exec_ir(size_t n) {
    ...
    std::cout << "\tin goto: pc = " << cur_ctx->pc << std::endl;
            ...
            case Operator::load: {
                ...
                if (inst->des.type == Type::Int && inst->op1.type == Type::IntPtr) {
                    DPRINT(find_src_operand(inst->op1)._val.iptr[off]);
