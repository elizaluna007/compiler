...
ir::Value ir::Executor::find_src_operand(Operand op)
{
    ...
    cout << "in find_src_operand 2" << endl;

    auto iter = cur_ctx->mem.find(op.name);

    cout << "which is finding:                     " << op.name << endl;

    cout << "in find_src_operand 2.5" << endl;
    for (auto it = cur_ctx->mem.begin(); it != cur_ctx->mem.end(); it++)
    {
        cout << "in find_src_operand 2.5.1" << endl;
        cout << it->first << endl;
    }
    if (iter == cur_ctx->mem.end())
    {
        cout << "in find_src_operand 2.6" << endl;
        iter = global_vars.find(op.name);
        cout << "in find_src_operand 2.7" << endl;
        assert(iter != global_vars.end() && "can not find the arguement in current conxtext or global variables");
    }

    cout << "in find_src_operand 3" << endl;

    retval = iter->second;

    cout << "in find_src_operand 4" << endl;

    assert(retval.t == op.type && "type not match");

    cout << "in find_src_operand 5" << endl;
#if (DEBUG_EXEC_DETAIL)
    std::cout << ", value = ";
    switch (retval.t)
    {
...
int ir::Executor::run()
{
    ...
    cout << "init global variables done" << endl;

    cout << "find main function and set cur_cxt" << endl;
    ...
    cout << "find main function and set cur_cxt done" << endl;
    ...
    cout << "push cur_cxt to cxt_stack" << endl;
    // run
    ...
    cout << "push cur_cxt to cxt_stack done" << endl;
